...
def check():
    ...
    remote=manifest.get("version",APP_VERSION)
    changed=[]
    for item in manifest.get("files",[]):
        local=APP_DIR/Path(item["path"])
        if not local.exists() or sha256(local)!=item["sha256"]: changed.append(item)
...
def apply(result):
    ...
    with tempfile.TemporaryDirectory() as td:
        for item in changed:
            ...
            if sha256(temp)!=item["sha256"]: raise RuntimeError(f"Checksum verification failed: {rel}")
            if target.exists():
...
def restart():
    cmd=[sys.executable,str(APP_DIR/"desktop.py")]
