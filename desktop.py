...
class FaceSorter(tk.Tk):
    def __init__(self):
        ...
        self.folder=tk.StringVar(); self.status=tk.StringVar(value="Choose a photo folder to begin."); self.groups=[]; self.group_signature=[]; self.photos=[]; self.photo_index=0; self.preview_image=None
    ...
    def start(self):
        ...
        threading.Thread(target=service.scan,args=(path,),daemon=True).start()
    def refresh(self):
        ...
        def worker():
            try:
                ...
                if not result.get("update"):
                    self.after(0,lambda:messagebox.showinfo("Face Sorter",f"You are up to date (v{result.get('current')})."))
                    return
