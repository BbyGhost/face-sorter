"""Face Sorter v3.3 enhanced launcher.
Recycled Canvas gallery, stable selection, debounced search, sorting, background exports,
and persistent face-memory preservation for destructive person operations.
"""
...
BORDER=desktop.BORDER; TEXT=desktop.TEXT; MUTED=desktop.MUTED; ACCENT=desktop.ACCENT; ACCENT_H=desktop.ACCENT_H; WHITE=desktop.WHITE
...
def _reset_with_memory():
    ...
    with service.DB_LOCK:
        service.DB.execute('DELETE FROM person_embeddings'); service.DB.commit()
    return result
...
class EnhancedFaceSorter(desktop.FaceSorter):
    ...
    def _gallery_area(self,main):
        ...
        self.sort_box=ttk.Combobox(tools,textvariable=self.sort_mode,state='readonly',width=16,values=('Most photos','Name A–Z','Name Z–A','Fewest photos'))
        self.sort_box.pack(side='left',padx=(0,10)); self.sort_box.bind('<<ComboboxSelected>>',lambda e:self.render_gallery())
        searchwrap=tk.Frame(tools,bg=PANEL2,highlightthickness=1,highlightbackground=BORDER); searchwrap.pack(side='left')
        tk.Label(searchwrap,text='⌕',bg=PANEL2,fg=MUTED,font=('Segoe UI',14)).pack(side='left',padx=(9,3))
        searchbox=tk.Entry(searchwrap,textvariable=self.search,bg=PANEL2,fg=TEXT,insertbackground=TEXT,relief='flat',bd=0,font=('Segoe UI',9),width=25)
        searchbox.pack(side='left',ipady=8,padx=(0,8)); self.search.trace_add('write',self._search_changed)
        ...
        self.canvas.bind('<MouseWheel>',self._wheel); self.canvas.bind('<Button-1>',self._gallery_click); self.canvas.bind('<Double-Button-1>',self._gallery_double_click)
        self.canvas.bind('<Configure>',lambda e:self._schedule_gallery_layout())
        self._gallery_after=None; self._gallery_generation=0; self._gallery_jobs={}; self._empty_gallery()
    ...
    def _sort_groups(self,groups):
        ...
        if mode=='Name A–Z': return sorted(groups,key=lambda g:(g.get('name') or '').lower())
        if mode=='Name Z–A': return sorted(groups,key=lambda g:(g.get('name') or '').lower(),reverse=True)
        if mode=='Fewest photos': return sorted(groups,key=lambda g:(int(g.get('photos') or 0),str(g.get('name') or '').lower()))
        return sorted(groups,key=lambda g:(-int(g.get('photos') or 0),str(g.get('name') or '').lower()))

    def render_gallery(self):
        query=self.search.get().strip().lower()
        self.filtered_groups=self._sort_groups([g for g in self.groups if not query or query in str(g.get('name') or '').lower()])
        self.people_count.config(text=f'{len(self.filtered_groups):,} people')
        self._gallery_generation+=1; self._gallery_jobs.clear(); self.canvas.yview_moveto(0)
    ...
    def _build_gallery_pool(self):
        ...
        self.canvas.configure(scrollregion=(0,0,width,max(1,rows_total*(card_h+gap)+20)))
        needed=min(120,max(cols*(int(height//(card_h+gap))+4),cols*5))
        while len(self._gallery_slots)<needed:self._gallery_slots.append(self._new_gallery_slot())
    ...
    def _layout_gallery_pool(self,cols,card_w=170,card_h=235,gap=12):
        top=self.canvas.canvasy(0); height=max(320,self.canvas.winfo_height()); first=max(0,int(top//(card_h+gap))-1)
        rows=max(2,int(height//(card_h+gap))+3); start=first*cols; end=min(len(self.filtered_groups),start+rows*cols)
        ...
        for slot_index,slot in enumerate(self._gallery_slots):
            ...
            g=self.filtered_groups[data_index]; ident=int(g['id']); r,c=divmod(data_index,cols); x=8+c*(card_w+gap); y=10+r*(card_h+gap)
            slot['person']=ident; self._gallery_slot_person[slot['bg']]=ident; self._gallery_slot_person[slot['panel']]=ident; self._gallery_slot_person[slot['thumb']]=ident; self._gallery_slot_person[slot['name']]=ident; self._gallery_slot_person[slot['count']]=ident
            for key in ('bg','panel','thumb','name','count'): self.canvas.itemconfigure(slot[key],state='normal')
            self.canvas.coords(slot['bg'],x,y,x+card_w,y+card_h); self.canvas.coords(slot['panel'],x+6,y+6,x+card_w-6,y+176); self.canvas.coords(slot['thumb'],x+card_w/2,y+91); self.canvas.coords(slot['name'],x+10,y+190); self.canvas.coords(slot['count'],x+10,y+211)
            self.canvas.itemconfigure(slot['name'],text=str(g['name'])); self.canvas.itemconfigure(slot['count'],text=f"{int(g['photos']):,} photos"); self._style_slot(slot,ident)
            photo=self._thumb_cache.get(ident)
            if photo: slot['photo']=photo; self.canvas.itemconfigure(slot['thumb'],image=photo,state='normal')
            else:
                slot['photo']=None; self.canvas.itemconfigure(slot['thumb'],image='',state='hidden')
                if ident not in self._gallery_jobs:
                    self._gallery_jobs[ident]=self._gallery_generation; threading.Thread(target=self._load_thumb,args=(ident,),daemon=True).start()

    def _hide_slot(self,slot):
        slot['person']=None
        for key in ('bg','panel','thumb','name','count'): self.canvas.itemconfigure(slot[key],state='hidden')

    def _style_slot(self,slot,ident):
        selected=ident in self.selected_ids; self.canvas.itemconfigure(slot['bg'],fill='#251b3a' if selected else CARD,outline=ACCENT if selected else BORDER)

    def _load_thumb(self,ident):
        try:
            path=service.person_thumbnail(ident)
            if not path:return
            with Image.open(path) as im:
                im=ImageOps.exif_transpose(im).convert('RGB'); im=ImageOps.fit(im,(158,158),method=Image.Resampling.LANCZOS); self._thumb_queue.put((ident,im.copy()))
        except Exception: pass

    def _poll_thumbnails(self):
        try:
            for _ in range(8):
                ident,im=self._thumb_queue.get_nowait(); photo=ImageTk.PhotoImage(im); self._thumb_cache[ident]=photo
                while len(self._thumb_cache)>self._thumb_cache_limit:self._thumb_cache.pop(next(iter(self._thumb_cache)))
                for slot in self._gallery_slots:
                    if slot['person']==ident: slot['photo']=photo; self.canvas.itemconfigure(slot['thumb'],image=photo,state='normal')
    ...
    def _select_all_key(self,event=None):
        self.selected_ids={int(g['id']) for g in self.filtered_groups}; self._update_selection_visuals(); return 'break'

    def _clear_selection_key(self,event=None):
        self.selected_ids.clear(); self._update_selection_visuals(); return 'break'
    ...
    def _bottom_bar(self,main):
        ...
        self.button(bar,'Select all',self._select_all_key).pack(side='left',padx=(14,4)); self.button(bar,'Clear',self._clear_selection_key).pack(side='left',padx=4); self.button(bar,'Rename',self.rename).pack(side='left',padx=4); self.button(bar,'Consolidate',self.consolidate_existing).pack(side='left',padx=4); self.button(bar,'Merge selected',self.merge_selected).pack(side='left',padx=4); self.button(bar,'Check for updates',self.check_updates).pack(side='left',padx=4)
        self.button(bar,'Export selected',self.export_selected).pack(side='right'); self.button(bar,'Export all people',self.export_all,True).pack(side='right',padx=6); self.button(bar,'Export unknown',self.export_unknown).pack(side='right',padx=6)
    ...
    def _background_export(self,kind,ids=None):
        ...
        def worker():
            try:
                result=service.export_person(ids[0],folder) if kind=='selected' else service.export_all_people(folder) if kind=='all' else service.export(service.ExportRequest(output_folder=folder))
                self.after(0,lambda r=result:messagebox.showinfo('Export complete',f"Copied {r.get('copied',0):,} photo(s).")); self.after(0,lambda:self.status.set('Export complete.'))
            except Exception as e:self.after(0,lambda e=e:messagebox.showerror('Export failed',str(e)))
    ...
    def show_settings(self):
        messagebox.showinfo('Face Sorter settings','Performance profile\n\n• Recycled People gallery (only a small pool of Canvas items)\n• Background thumbnail decoding with a bounded cache\n• Debounced search and stable selection\n• Sort by photos or name\n• Background exports\n• CPU / GPU / CPU + GPU modes\n• Persistent face prototypes across future scans and merges\n\nEverything stays local on this computer.')
