"""Face Sorter stable launcher v3.3.2.
Fixes Tkinter initialization order while retaining the recycled People gallery.
"""
...
class EnhancedFaceSorter(desktop.FaceSorter):
    def __init__(self):
        # Do not create Tk variables here: FaceSorter/tk.Tk has not initialized
        # the Tcl interpreter until super().__init__() runs.
    ...
    def _gallery_area(self,main):
        # super().__init__() has already created the Tk root at this point.
        self.sort_mode=tk.StringVar(master=self,value='Most photos')
    ...
    def _layout_gallery_pool(self,cols,card_w=170,card_h=235,gap=12):
        ...
        rows=max(2,int(height//(card_h+gap))+3)
        ...
        for slot_index,slot in enumerate(self._gallery_slots):
            ...
            for key in ('bg','panel','thumb','name','count'):
                self._gallery_slot_person[slot[key]]=ident
                self.canvas.itemconfigure(slot[key],state='normal')
            slot['person']=ident
            ...
            self.canvas.coords(slot['name'],x+10,y+190)
            self.canvas.coords(slot['count'],x+10,y+211)
            self.canvas.itemconfigure(slot['name'],text=str(g['name']))
            self.canvas.itemconfigure(slot['count'],text=f"{int(g['photos']):,} photos")
            ...
            if photo:
                ...
                if ident not in self._gallery_jobs:
                    ...
                    threading.Thread(target=self._load_thumb,args=(ident,),daemon=True).start()

    def _hide_slot(self,slot):
        slot['person']=None; slot['photo']=None
        for key in ('bg','panel','thumb','name','count'):
            self._gallery_slot_person.pop(slot[key],None)
    ...
    def _load_thumb(self,ident):
        try:
            ...
            with Image.open(path) as im:
                ...
                im=ImageOps.fit(im,(158,158),method=Image.Resampling.LANCZOS)
    ...
    def _select_all_key(self,event=None):
        self.selected_ids={int(g['id']) for g in self.filtered_groups}
        self._update_selection_visuals(); return 'break'
    ...
    def _background_export(self,kind,ids=None):
        ...
        def worker():
            try:
                ...
                self.after(0,lambda r=result:messagebox.showinfo('Export complete',f"Copied {r.get('copied',0):,} photo(s)."))
